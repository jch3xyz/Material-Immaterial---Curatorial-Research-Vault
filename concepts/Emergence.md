...
The same picture admits two further degrees and a counter-current. Luc Steels (via Hayles) distinguishes *first-order emergence* from *second-order emergence*, which privileges emergent properties that add functionality, "particularly the ability to process information," so that "the organism has in effect evolved the capacity to evolve"; Rodney Brooks (via Hayles) engineers emergence in embodied hardware, where Genghis's gait "is not programmed in advance" but emerges as the legs take account of one another, and consciousness itself is recast as "an emergent property" — "a 'cheap trick.'" Emergence is what makes the [[The Computational Universe|computational universe]] persuasive — life and computation are claimed to converge "at the edge of chaos" — but because it operates on substrate-independent code ("the organism is the code, and the code is the organism"), it slides easily into the disembodied reading of [[Information]] Hayles resists (see [[The Body as Data]], [[Embodiment vs Information]]). She does not reject emergence; she contests the framework that treats emergent informational pattern as more real than the [[Embodiment|embodied]] materiality it runs on — the fault line on which the corpus's other readings divide. [[Meghan O'Gieblyn]] presses it from the side of suspicion, reading emergence as vitalism's structural heir and, beneath the mystical surface, a reductive materialism that "end[s] up dismissing the very thing they are supposed to explain." [[Ray Kurzweil]] presses it from the side of engineering, accepting that "a completely deterministic process can produce results that are completely unpredictable" but insisting on its ceiling and on its tractability to mathematics. And the art register — [[Jack Burnham]]'s object-to-system migration and [[Roy Ascott]]'s self-organising artwork and emergent network — relocates the leap "into the new" from AL code to the cybernetic artwork and the planetary field of mind, grounding it (like the AL optimists, against O'Gieblyn's charge) in ascending complexity and feedback rather than a vital essence. Each section below develops one of these readings.
...
- [[Self-Organization]] — the second-wave springboard; emergence is what self-organization becomes when it leaps "outward into the new" rather than merely reproducing internal order.
