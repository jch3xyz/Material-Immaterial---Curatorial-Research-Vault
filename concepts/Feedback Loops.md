...
The feedback loop is the cybernetic mechanism by which information circulates between a system and its environment (or among a system's components) so that an output re-enters as an input and the system regulates itself. In the corpus the loop does double duty: it is the self-regulating, equilibrium-seeking circuit of control theory, and — in [[N. Katherine Hayles]]'s account — the device that renders the boundaries of the autonomous liberal subject "up for grabs" by letting control and information flow across the skin rather than stopping at it.

## General Orientation

"Feedback" names a single mechanism — output re-entering as input so a system regulates itself — but the corpus puts it to sharply different work, and the word *equilibrium* is the hinge on which the readings turn and divide. The sources sort into three stances.
...
- **[[Norbert Wiener]]** — The originating figure of the connective loop. Because the universe is probabilistic, control requires "a flexible, self-regulating system of control based on feedback from the system itself"; teleological behavior is redefined as "behavior controlled by negative feedback." Hayles reads Wiener as wanting to preserve liberal subjectivity even as his own feedback logic undermines it.
